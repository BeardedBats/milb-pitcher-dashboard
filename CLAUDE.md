...
- **Two cache versions, not one.** `CARD_SCHEMA_VERSION` (data.py) covers the daily agg keys; `_METRICS_VERSION` (boxscore_levels.py) covers the per-game derived rows and pitch metrics, which have a 30-day TTL and sit BEHIND the daily keys. Changing a derived-row shape and bumping only `CARD_SCHEMA_VERSION` makes the daily key miss, recompute, and read stale rows straight back out — bump BOTH.
