...
- `warmup-daily` loops all 6 levels under a deadline, then warms the AAA/AFL **box-score** rows the All-Levels filter reads (the loop above builds only their Statcast rows) — last, so a slow morning cuts that rather than a single-level homepage. Its past-week pass warms box rows at every level for the same reason; it evicts the raw feed L1 between days. `warmup-daily-cards` and both live crons are AAA+AFL only (no cards exist elsewhere).
...
- `materialize-ranges` (every 5 min) also runs, in priority order after the drain: the ledger advance, the Rehab warm, and `_backfill_daily_slates` — a Redis cursor (`backfill:cursor:s{VER}m{METRICS}`) that walks the per-day homepage caches backward from today-8 to `SEASON_START`, ~2 cold days per tick, so the CSV export's day-by-day loop (and deep date paging) hits warm caches. Converged state is one GET per tick; a version bump on either cache restarts the walk. Two invariants (`test_backfill_eviction.py`): `evict_local_day` after every warmed day — `fetch_date` pins each past day's frame in L1 forever, so a long walk in ONE warm cron instance otherwise assembles the season frame and OOMs (~hourly, seen in prod on day one) — and the cursor persists after EACH day so a killed tick keeps its progress.
...
- **`ALL` is a pseudo-level, not a level.** The leaderboard's "All Levels" filter (`levels.ALL_LEVELS`) is deliberately absent from `LEVELS` and `LEVEL_ORDER` — it has no sportId, no schedule and no team map. `normalize_level` therefore coerces it to **AAA**, so every handler must ask `is_all_levels()` *first*; normalize-then-check answers an all-levels request with Triple-A and does it silently. `/api/pitcher-results` and `/api/initial-load` fold it via `_all_levels_results`; `/api/games` and `/api/pitch-data` answer it with nothing (a game tab and a pitch table each belong to one level, so All-Levels is leaderboard-only and the frontend hides the tab strip). See `backend/tests/test_all_levels_filter.py`.
- **All-Levels uses the box-score path at EVERY level**, AAA and AFL included — not Savant rows for those two and feed rows for the rest. One table means one definition per column: SwStr% has to be the same measurement in a AAA row and an A+ row or the sort is a lie. It reads the same `daily_results_box_{level}_...` key the single-level pages write (`_box_results_for_level`), so the two views share warm data, and it folds levels **sequentially**, dropping each slate's raw feed payloads (`evict_local_transients`) before the next — six slates of raw feeds in one instance is the memory shape that has OOM-killed warm loops before. AAA's Statcast columns are not lost; they are what the AAA view is for.
...
## Rates that cross levels — the numerator and denominator must be siblings
A game log is the one place two producers meet: Savant supplies AAA/AFL rows, the play-by-play feed (`boxscore_levels._derive_pitch_metrics`) supplies everything below. A season rate is only honest if BOTH of its components come from both.
- **2Str% and PAR% are PLATE-APPEARANCE rates**, not pitch rates: `two_strike_pas / pa_count` and `ks / two_strike_pas`. The feed path once published `two_strike_pas` with no `pa_count` and a pitch-based `two_str_pct` (`two_strike_pitches / pitches`) under the same column name — so a AA→AAA season line summed every level's numerator over AAA-only PAs and printed **115%**. The pitch-count version still exists as `two_str_pitch_pct`; `two_strike_pitches` is a raw count, not the column.
- Both feed-side PA counters credit whoever **finished** the PA, and `pa_count` uses that identical rule on purpose — that is what makes `two_strike_pas <= pa_count` true per row, which is what makes any sum of them safe. A play with no pitch (pickoff, runner event) is not a PA.
- The totals math (`season.aggregate_game_log_to_totals`, `frontend/src/utils/seasonTotals.js`) pairs them per game: a row with no `pa_count` sits out of 2Str% entirely rather than contributing a numerator alone. PAR% deliberately does NOT pair — its numerator (`ks`) comes off the box score and exists at every level, so pairing would shrink only the denominator. Pinned by `backend/tests/test_two_strike_totals.py` + `frontend/src/utils/__tests__/seasonTotals.test.js`.
- Per-pitch-type "2-Str" in the Pitch Mix / Usage views IS a pitch rate and is correct — a pitch type has no plate appearances. Different column, different denominator.
...
- `frontend/src/components/AdaptedResultsTable.jsx` — Box-score results table for non-Statcast levels **and for the All-Levels filter**: Date+Lvl | Pitcher | Team | Opp | Dec | IP | H | R | ER | BB | K | HR | BF | P | Str% | GO/AO. No derived metrics beyond Str% and GO/AO.
...
### The season ledger — fold forward, never re-fold
`backend/ledger.py` holds the season-to-date answer as ONE pickled state blob in Redis: running per-team results/pitch accumulators + the directory accumulator + a high-water mark (`asof`). The 5-minute materialize cron calls `advance_ledger`, which folds exactly the days after `asof` in date order — steady state is one day per morning; an initial or post-override build spreads across a few ticks via checkpoints.
- Readers (`team_season_rows`, `aaa_rows_for_teams`, `directory_rows`) finalize a DEEP COPY and layer today's cached day frame on top. They return **None unless `asof` == yesterday**, and every caller treats None as "use the old fold/202 path" — a behind ledger must never serve a silently short stat line. The old paths are the fallback, not dead code.
- Pickle+gzip on purpose: the accumulators use tuple keys, sets and numpy scalars that JSON cannot round-trip. Private Redis, our own writer/reader — pickle's trust model is fine here.
- The key embeds `CARD_SCHEMA_VERSION`; the state embeds the override version. Either changing rebuilds the ledger from per-day snapshots (cheap — Redis reads). Stat corrections to past days are deliberately NOT reflected until a rebuild; accepted staleness.
- Days fold exactly once, strictly in order, and the mark moves only after the fold — that invariant is what keeps every sum honest. Never fold a day twice, never skip one.
- Past-date responses carry `s-maxage=2592000` so the CDN serves last week's slates without invoking the function at all.
