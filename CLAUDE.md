...
- **`RANGE_DAY_TTL` (60 days) is shorter than a season (~190 days).** The earliest days therefore expire while the season is still running, so a season-wide range cannot stay materialized on its own — it depends on the daily re-queue re-baking what expired. That works, but it means a permanently churning tail. Raising the TTL past a full season would stop the churn at the cost of Upstash storage.
